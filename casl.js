...
function Bits64K() {
	...
	this.initialize = function() {
		for (var i = 0; i < this.ar.length; i++) this.ar[i] = 0;
		this.count = 0;
	}
...
function ParsedLine() {
	...
	this.isEmpty = function() {
		if (this.label == "" && this.inst == "" && this.operands.length == 0) return true;
		else return false;
	}
}
...
function assembleOneLine(parsedLine) {
	...
	if (parsedLine.inst == "START") aiTableItem.callFnc(parsedLine);
	else if (parsedLine.inst == "END") aiTableItem.callFnc(parsedLine);
	else {
		if (startLabel == null) {
			logError(lineNumber, "STARTがありません");
			processStart("", "");
		}
		if (parsedLine.label != "") defineLabel(parsedLine.label);
...
function alStart(code, parsedLine) {
	...
	if (parsedLine.label == "") {
		...
		label = "";
	} else if (isLabel(parsedLine.label)) label = parsedLine.label;
	if (parsedLine.operands.length >= 1) {
		if (isLabel(parsedLine.operands[0])) operand = parsedLine.operands[0];
		else logError(lineNumber, "オペランドが不正です - " + parsedLine.operands[0]);
		if (parsedLine.operands.length > 1) logError(lineNumber, "余計なオペランドがあります");
	}
...
function alEnd(code, parsedLine) {
	...
	if (parsedLine.label != "") logError(lineNumber, "END命令にはラベルはつけられません");
	if (parsedLine.operands.length != 0) logError(lineNumber, "余計なオペランドがあります");
	processEnd();
}
function processEnd() {
	...
	if (startLabel != "")	{
		...
		if (startOperand != "") {
			...
			else logError(startLine, "未定義です - " + startOperand);
		}
		addressTable[startLine] = startLocation;
		defineGlobalLabel(startLabel, startLocation, startLine);
		if (entryPoint < 0) entryPoint = startLocation;
	}
	for (var itemname in localLabelTable) {
		var item = localLabelTable[itemname];
		if (item.value < 0) for (var i = 0; i < item.refAddr.length; i++) referGlobalLabel(itemname, item.refAddr[i], item.refLine[i]);
		else if (item.multiDefined) {
			logError(item.defLine, "多重定義されています - " + itemname);
			for (var i = 0; i < item.refLine.length; i++) logError(item.refLine[i], "多重定義ラベルを参照しています - " + itemname);
		}
		else for (var i = 0; i < item.refAddr.length; i++) objectcode[item.refAddr[i]] = item.value;
	}
	startLabel = null;
	startOperand = null;
	startLine = -1;
...
function resolveGlobalReferences(entryPointName) {
	for (var itemname in globalLabelTable) {
		...
		if (item.value < 0) for (var i = 0; i < item.refLine.length; i++) logError(item.refLine[i], "未定義です - " + itemname);
		else if (item.multiDefined) {
			logError(item.defLine, "多重定義されています - " + itemname);
			for (var i = 0; i < item.refLine.length; i++) logError(item.refLine[i], "多重定義ラベルを参照しています - " + itemname);
		}
	...
	if (entryPointName != "") {
		...
		if ((!item) || item.value < 0) alert("実行開始ラベル欄で指定された" + entryPointName + "がプログラム中に存在しません");
		if (item) entryPoint = item.value;
	}		
}
...
function alRMR2(code, parsedLine, rrOK) {
	...
	var addr = 0;
	var xr = 0;
	if (parsedLine.operands.length < 2) logError(lineNumber, "オペランドの数が不足です");
	else {
...
function alM(code, parsedLine) {
	var addr = 0;
	var xr = 0;
	if (parsedLine.operands.length < 1) logError(lineNumber, "オペランドの数が不足です");
	else {
...
function alDS(code, parsedLine) {
	var count = 0;
	if (parsedLine.operands.length != 1) logError(lineNumber, "オペランドの数が不正です");
	else {
		var str = parsedLine.operands[0];
		if (str.match(/^[0-9]+$/)) count = str - 0;
		else logError(lineNumber, "オペランドが不正です - " + str);
	}
	addressTable[lineNumber] = locationCounter;
	nonExecutableTable[lineNumber] = true;
...
function alDC(code, parsedLine) {
	if (parsedLine.operands.length == 0) logError(lineNumber, "オペランドがありません");
	else {
		addressTable[lineNumber] = locationCounter;
		nonExecutableTable[lineNumber] = true;
		for (var i = 0; i < parsedLine.operands.length; i++) {
			var str = parsedLine.operands[i];
			if (str.length == 0) logError(lineNumber, "オペランドが不正です");
			else if (!allocConstant(str)) logError(lineNumber, "オペランドが不正です - " + str);
		}
	}
}
function allocConstant(str) {
	var val = 0;
	if (isLabel(str)) referLabel(str, locationCounter);
	else {
		val = getDec(str);
		if (val < 0) val = getHex(str);
	}
	if (val >= 0) {
	...
	var ar = getStr(str);
	if (ar) {
		for (var j = 0; j < ar.length; j++) objectcode[locationCounter++] = ar[j];
		return true;
	}
	return false;
}
...
function alRPush(code, parsedLine) {
	...
	addressTable[lineNumber] = locationCounter;
	for (var i = 1; i <= 7; i++) {
		if (i != 1) macroFlag.set(locationCounter);
...
function alRPop(code, parsedLine) {
	...
	addressTable[lineNumber] = locationCounter;
	for (var i = 7; i >= 1; i--) {
		if (i != 7) macroFlag.set(locationCounter);
...
function parseLine(str) {
	...
	while (true) {
		...
		if (!token.getToken(str) || token.token == " " || token.token == ",") {
...
function getXRCheck(operands, n) {
	...
	if (operands.length > n + 1) logError(lineNumber, "余計なオペランドがあります");
	return xr;
}
function getAddrCheck(str) {
	...
	if (str.charAt(0) == '=') {
		...
		if (getDec(str1) >= 0 || getHex(str1) >= 0 || getStr(str1) != null) {
			referLabel(str, locationCounter + 1);
			return 0;
		}
...
function getDec(str) {
	if (str.match(/^[+-]?[0-9]+$/)) return (str - 0) & 0xFFFF;
	return -1;
}
function getHex(str) {
	if (str.match(/^#[0-9A-Fa-f]+$/)) return parseInt(str.substring(1), 16) & 0xFFFF;
	return -1;
}
function getStr(str) {
	...
	for (var i = 1; i < str.length - 1; i++) {
		...
		if (ch == '\'') {
			...
			if (jis8 >= 0) arCharCode.push(jis8);
			else arCharCode.push("?".charCodeAt(0));
		}
	}
	return arCharCode;
}
...
function setState(state) {
	...
	for (var i = 0; i < 8; i++) setReadOnly("GR" + i, r);
	setReadOnly("PR", r);
	setReadOnly("SP", r);
	setReadOnly("ZF", r);
	setReadOnly("SF", r);
	setReadOnly("OF", r);
	for (var i = 0; i < 16; i++) setReadOnly("mem_" + i, r);
...
function setReadOnly(id, readonly) {
	...
	if (elem.readOnly != readOnly) elem.readOnly = readOnly;
}
var gr = new Array(8);
var sp;
var pr;
...
function initializeRegMem() {
	for (var i = 0; i < 8; i++) gr[i] = 0;
	sp = spInit;
	pr = entryPoint;
	zf = sf = of = 0;
	for (var i = 0; i < 65536; i++) mem[i] = objectcode[i];	
...
function outputReg() {
	...
	for (var i = 0; i < 8; i++) document.getElementById("GR" + i).value = fnc(gr[i]);
...
function outputMem() {	
	...
	var fnc = getRadix(hex4, unsignedDec, signedDec);
	var addr = getAddress();
	for (var i = 0; i < 16; i += 4) document.getElementById("addr_" + i).innerHTML = hex4(ea(addr, i));
	for (var i = 0; i < 16; i++) document.getElementById("mem_" + i).value = fnc(mem[ea(addr, i)]);
}
...
function inputReg() {
	...
	for (var i = 0; i < 8; i++) gr[i] = fnc(document.getElementById("GR" + i).value);
...
function inputMem() {
	var fnc = getRadix(fromHex, fromDec, fromDec);
	var addr = getAddress();
	for (var i = 0; i < 16; i++) mem[ea(addr, i)] = fnc(document.getElementById("mem_" + i).value);
}
...
function getRadix(hex, unsigned, signed) {
	if (radix == 0) return hex;
	if (radix == 1) return unsigned;
	if (radix == 2) return signed;
}
function getAddress() {
	return address;
}
function onClickRadix() {
	inputRegMem();
	if (document.getElementById("radix_hex").checked) radix = 0;
	if (document.getElementById("radix_unsigned").checked) radix = 1;
	if (document.getElementById("radix_signed").checked) radix = 2;
	outputRegMem();
}
...
function ConsoleView() {
	...
	this.update = function() {
		...
		for (var i = this.arrayLines.length; i < this.lines; i++) filler.push("<br />");
	...
	this.outputOneLine = function(start, len) {
		...
		for (var i = 0; i < len; i++) {
			...
			if (charCode >= 0) {
				if (charCode <= 0x20 || charCode == 0x7F) charCode = 0x20;
				arrayChars.push(String.fromCharCode(charCode));
			} else arrayChars.push('?');
		}
		var str = enCER(arrayChars.join(""));
		while (this.arrayLines.length >= this.lines) this.arrayLines.shift();
...
function stepOut() {
	if (stateCurrent != STATE_BREAK) return;
	breakOnRet = true;		
	callNest = 0;
	inputRegMem();
...
function cont() {
	for (var i = 0; i < 100 && stateCurrent == STATE_RUNNING; i++) singleStep();
	if (stateCurrent == STATE_RUNNING) setTimeout("cont();", 1);
	else outputRegMem();
}
function pause() {
	if (stateCurrent == STATE_RUNNING) setState(STATE_BREAK);
}
function stop() {
...
function singleStep() {
	do {
		...
		if (stateCurrent == STATE_UNSTARTED) return;
